...
#define TT_IO_MAP_INLINE inline __attribute__((always_inline))
#else
#define TT_IO_MAP_INLINE __forceinline
#endif
...
TT_IO_MAP_INLINE uint32_t get_operand_stream_id(int operand) {
...
TT_IO_MAP_INLINE volatile uint32_t* get_cb_tiles_received_ptr(int operand) {
    return (volatile uint32_t*)(uintptr_t)(STREAM_REG_ADDR(
        get_operand_stream_id(operand), STREAM_REMOTE_DEST_BUF_SIZE_REG_INDEX));
}

TT_IO_MAP_INLINE volatile uint32_t* get_cb_tiles_acked_ptr(int operand) {
    return (volatile uint32_t*)(uintptr_t)(STREAM_REG_ADDR(
        get_operand_stream_id(operand), STREAM_REMOTE_DEST_BUF_START_REG_INDEX));
}

TT_IO_MAP_INLINE volatile uint32_t* get_cq_finish_ptr() {
    return (volatile uint32_t*)(uintptr_t)(STREAM_REG_ADDR(
        get_operand_stream_id(0), STREAM_REMOTE_DEST_BUF_START_REG_INDEX));
}

TT_IO_MAP_INLINE volatile uint32_t* get_sync_register_ptr() {
